...
export async function onRequestGet(context) {
  ...
  if (!envelope.certificate_pdf_url) {
    return new Response(JSON.stringify({ error: "not_ready", detail: "Certificate is still being generated" }),
      { status: 425, headers: corsHeaders });
  }

  const { data: signed, error: signErr } = await supabase.storage
    .from("pdfs")
    .createSignedUrl(envelope.certificate_pdf_url, 300);
