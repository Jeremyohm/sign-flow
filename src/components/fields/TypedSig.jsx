...
export function TypedSig({ name, setName, onCapture }) {
  ...
  const render = useCallback(v => {
    ...
    ctx.fillStyle = T.text; ctx.font = "40px Caveat, cursive"; ctx.textBaseline = "middle"; ctx.fillText(v, 20, 60);
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 10, height: 72, background: T.white, borderRadius: 10,
        ...
        {name.trim() ? <span style={{ fontFamily: F.sig, fontSize: 34, color: T.text }}>{name}</span>
